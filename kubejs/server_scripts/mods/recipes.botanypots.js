onEvent("recipes", (event) => {
    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:cobalt_seeds",
        },
        categories: ["imperium"],
        growthTicks: 4200,
        display: {
            block: "mysticalagriculture:cobalt_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:cobalt_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:cobalt_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:draconium_seeds",
        },
        categories: ["supremium"],
        growthTicks: 3600,
        display: {
            block: "mysticalagriculture:draconium_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:draconium_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:draconium_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:awakened_draconium_seeds",
        },
        categories: ["insanium"],
        growthTicks: 4600,
        display: {
            block: "mysticalagriculture:awakened_draconium_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:awakened_draconium_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:awakened_draconium_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:apatite_seeds",
        },
        categories: ["prudentium"],
        growthTicks: 2400,
        display: {
            block: "mysticalagriculture:apatite_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:apatite_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:apatite_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:marble_seeds",
        },
        categories: ["prudentium"],
        growthTicks: 2400,
        display: {
            block: "mysticalagriculture:marble_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:marble_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:marble_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:limestone_seeds",
        },
        categories: ["prudentium"],
        growthTicks: 2400,
        display: {
            block: "mysticalagriculture:limestone_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:limestone_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:limestone_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:basalt_seeds",
        },
        categories: ["prudentium"],
        growthTicks: 2400,
        display: {
            block: "mysticalagriculture:basalt_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:basalt_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:basalt_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:tinkers_bronze_seeds",
        },
        categories: ["tertium"],
        growthTicks: 3600,
        display: {
            block: "mysticalagriculture:tinkers_bronze_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:tinkers_bronze_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:tinkers_bronze_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:slimesteel_seeds",
        },
        categories: ["tertium"],
        growthTicks: 3600,
        display: {
            block: "mysticalagriculture:slimesteel_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:slimesteel_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:slimesteel_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:pig_iron_seeds",
        },
        categories: ["tertium"],
        growthTicks: 3600,
        display: {
            block: "mysticalagriculture:pig_iron_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:pig_iron_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:pig_iron_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:sapphire_seeds",
        },
        categories: ["imperium"],
        growthTicks: 4000,
        display: {
            block: "mysticalagriculture:sapphire_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:sapphire_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:sapphire_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:ruby_seeds",
        },
        categories: ["imperium"],
        growthTicks: 4000,
        display: {
            block: "mysticalagriculture:ruby_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:ruby_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:ruby_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:rose_gold_seeds",
        },
        categories: ["imperium"],
        growthTicks: 4000,
        display: {
            block: "mysticalagriculture:rose_gold_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:rose_gold_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:rose_gold_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:hepatizon_seeds",
        },
        categories: ["supremium"],
        growthTicks: 4500,
        display: {
            block: "mysticalagriculture:hepatizon_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:hepatizon_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:hepatizon_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:queens_slime_seeds",
        },
        categories: ["supremium"],
        growthTicks: 4500,
        display: {
            block: "mysticalagriculture:queens_slime_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:queens_slime_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:queens_slime_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:manyullyn_seeds",
        },
        categories: ["supremium"],
        growthTicks: 4500,
        display: {
            block: "mysticalagriculture:manyullyn_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:manyullyn_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:manyullyn_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:nitro_crystal_seeds",
        },
        categories: ["insanium"],
        growthTicks: 4500,
        display: {
            block: "mysticalagriculture:nitro_crystal_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:nitro_crystal_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:nitro_crystal_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots
        .soil({
            input: Ingredient.of("miniutilities:ender_pearl_block").toJson(),
            display: {
                block: "miniutilities:ender_pearl_block",
            },
            categories: ["ender_pearl_block"],
            growthModifier: 0.5,
        })
        .id("kubejs:botany_pots/soil/ender_pearl_block");

    event.recipes.botanypots.crop({
        seed: {
            item: "miniutilities:ender_lily_seeds",
        },
        categories: ["ender_pearl_block", "end_stone"],
        growthTicks: 4200,
        display: {
            block: "miniutilities:ender_lily_block",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "minecraft:ender_pearl",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "miniutilities:ender_lily_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots
        .soil({
            input: Ingredient.of("botania:blaze_block").toJson(),
            display: {
                block: "botania:blaze_block",
            },
            categories: ["blaze_block"],
            growthModifier: 0.5,
        })
        .id("kubejs:botany_pots/soil/blaze_block");

    event.recipes.botanypots
        .soil({
            input: Ingredient.of("minecraft:netherrack").toJson(),
            display: {
                block: "minecraft:netherrack",
            },
            categories: ["netherrack"],
            growthModifier: 0.3,
        })
        .id("kubejs:botany_pots/soil/netherrack");

    event.recipes.botanypots.crop({
        seed: {
            item: "miniutilities:flame_lily_seeds",
        },
        categories: ["blaze_block", "netherrack"],
        growthTicks: 4200,
        display: {
            block: "miniutilities:flame_lily_block",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "miniutilities:flame_lily",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "miniutilities:flame_lily_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:electrotine_seeds",
        },
        categories: ["prudentium"],
        growthTicks: 2400,
        display: {
            block: "mysticalagriculture:electrotine_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:electrotine_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:electrotine_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });

    event.recipes.botanypots.crop({
        seed: {
            item: "mysticalagriculture:crystal_shards_seeds",
        },
        categories: ["tertium"],
        growthTicks: 2400,
        display: {
            block: "mysticalagriculture:crystal_shards_crop",
            properties: {
                age: 7,
            },
        },
        results: [
            {
                chance: 0.75,
                output: {
                    item: "mysticalagriculture:crystal_shards_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.05,
                output: {
                    item: "mysticalagriculture:crystal_shards_seeds",
                },
                minRolls: 1,
                maxRolls: 1,
            },
            {
                chance: 0.01,
                output: {
                    item: "mysticalagriculture:fertilized_essence",
                },
                minRolls: 1,
                maxRolls: 1,
            },
        ],
    });
});
